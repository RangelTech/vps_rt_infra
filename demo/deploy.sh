...
python3 -m pip install --quiet --user pyyaml kagglehub

# Secrets persist across deploys; only the first deploy generates them.
[ -f deploy/public-demo/.env ] || python3 scripts/public_demo_env.py
python3 scripts/public_demo_env.py --set EDUCATION_RELEASE_DIR=./data/education-release
...
if [ ! -f deploy/public-demo/data/education-release/release_manifest.json ]; then
  release_dir=$(python3 -c "import kagglehub; print(kagglehub.dataset_download('lucasrangelss/brazil-education-data-lake/versions/1'))")
  cp "$release_dir"/* deploy/public-demo/data/education-release/
fi

# Idempotent: no-ops if nginx is already patched.
python3 "$INFRA_DIR/patch_compose_for_traefik.py" deploy/public-demo/compose.yaml "$ROOT_DOMAIN"

python3 scripts/check_public_demo_policy.py --strict-release
...
uuid=$(printf '%s' "$seed_output" | tail -1 | python3 -c "import json,sys; print(json.load(sys.stdin)['public_uuid'])")
current_uuid=$(grep '^PUBLIC_DASHBOARD_UUID=' deploy/public-demo/.env | cut -d= -f2)
if [ "$uuid" != "$current_uuid" ]; then
  python3 scripts/public_demo_env.py --set "PUBLIC_DASHBOARD_UUID=$uuid"
